...
def generate_portal(output_dir, repo_name="Purdue ROV Hardware", commit_sha="", branch="master"):
    ...
    safe_commit_sha = html.escape(commit_sha[:7]) if commit_sha else ""
...
<head>
    ...
    <title>{safe_repo_name} | Hardware Design Portal</title>
    <style>
        :root {{
            --bg: #0f111a;
            --surface: #1e222d;
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="meta-badges">
                <span class="badge">Branch: <strong>{safe_branch}</strong></span>
                {f'<span class="badge">Commit: <code>{safe_commit_sha}</code></span>' if safe_commit_sha else ''}
...
"""
    ...
    has_artifacts = bool(ibom_link or sch_link or layout_link or bom_link)
    if not has_artifacts:
        html_content += """
            <div class="empty-state-card" style="grid-column: 1 / -1; text-align: center; padding: 2.5rem; background: #161b22; border-radius: 8px; border: 1px dashed var(--border);">
                <div style="font-size: 2rem; margin-bottom: 0.5rem;">📁</div>
                <div style="font-weight: 600; color: var(--accent); margin-bottom: 0.25rem;">No design artifacts detected</div>
                <div style="color: var(--text-muted); font-size: 0.85rem;">Outputs will appear here once generated by the KiBot CI pipeline.</div>
            </div>
"""
